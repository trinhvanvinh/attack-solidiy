...
contract TokenBalanceTracker {
    ...
    function logBalances(address _from) public {
        (
            BalanceDeltaReturn memory nativeTokenDelta,
            BalanceDeltaReturn[] memory tokensDelta
    ...
    function toStringWithDecimals(
        ...
        uint256 decimalsToPrint = _number -
            (_number / (10 ** decimals)) *
            (10 ** decimals);
        return
            string.concat(
                integerToPrint.toString(),
                ".",
                decimalsToPrint.toString()
            );
    }
    ...
    function calculateBalanceDelta(
        ...
        view
        returns (
            BalanceDeltaReturn memory nativeDelta,
            BalanceDeltaReturn[] memory tokenDeltas
        )
    {
        (
            uint256 prevNativeBalance,
            uint256[] memory prevTokenBalance
        ) = getBalanceTrackers(_user);
        ...
        uint256 tokensLength = trackedTokens.length;
        if (tokensLength > 0) {
            BalanceDeltaReturn[] memory memDeltas = new BalanceDeltaReturn[](
                tokensLength
            );
            for (uint i = 0; i < tokensLength; i++) {
                uint256 currentTokenBalance = IERC20Local(trackedTokens[i])
                    .balanceOf(_user);
